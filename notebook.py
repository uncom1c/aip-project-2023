...
attack60 = []
'''
Это не функция, но приближенное к ней
Работает только если получилось найти коэффиценты ГПСЧ
Сначала запускаем 60 раз уже свой генератор относительно найденных данных
Затем действуем по алгоритму расшифровки, брутфорсом перечисляем все возможные варианты ключа
Потом выводим все отдаленно похожие выводы, при которых ответ хотя бы существует
добавляем все эти выводы в список attack60
'''
if potential[0] != 0:
    for i in range(60):
